...
#if 0
/* The last key setup in hw */
static uint8_t key_in_hw[16] = { 0 };
#endif

static inline void local_cryp_aes_128_set_key(const uint8_t key[16])
{
#if 0
	/* We change the key and perform a "real" key schedule only if it is not the same */
	if(memcmp(key, key_in_hw, 16)){
		hal_cryp_aes_128_set_key(key);
		memcpy(key_in_hw, key, 16);
	}
#else
	hal_cryp_aes_128_set_key(key);
#endif
