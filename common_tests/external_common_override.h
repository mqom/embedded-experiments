#ifndef __EXTERNAL_COMMON_OVERRIDE_H__
#define __EXTERNAL_COMMON_OVERRIDE_H__

/* XXX: this file is used to remove the allocation related probing 
 * effect on measurements when malloc/free is heavily used ... */

/* External APIs for pausing and resuming cycles count */
extern void hal_pause_cycles(void);
extern void hal_unpause_cycles(void);
...
#define MQOM2_PARAM_OPENING_X1_SIZE (                                         \
...
#define MQOM2_PARAM_OPENING_SIZE (MQOM2_PARAM_TAU*MQOM2_PARAM_OPENING_X1_SIZE)
#define MQOM2_PARAM_OPENING_ONETREE_SIZE (MQOM2_PARAM_TAU*(                   \
    BYTE_SIZE_FIELD_BASE(MQOM2_PARAM_MQ_N)                                    \
      + MQOM2_PARAM_DIGEST_SIZE                                               \
) + (MQOM2_PARAM_TOPEN*MQOM2_PARAM_SEED_SIZE))

#if !defined(BLC_ONETREE)
...
#define MQOM2_SIG_SIZE (                                                      \
    4 + MQOM2_PARAM_TAU*BYTE_SIZE_FIELD_BASE(MQOM2_PARAM_ETA*MQOM2_PARAM_MU)  \
      + (MQOM2_PARAM_SALT_SIZE) + (2*(MQOM2_PARAM_DIGEST_SIZE))               \
      + MQOM2_PARAM_OPENING_ONETREE_SIZE                                      \
)
#endif
#define MQOM2_PARAM_PREOPENING_SIZE (                                         \
    MQOM2_PARAM_TAU*(                                                         \
        BYTE_SIZE_FIELD_BASE(MQOM2_PARAM_MQ_N)-MQOM2_PARAM_SEED_SIZE          \
    ) + MQOM2_PARAM_SEED_SIZE                                                 \
)
#define MQOM2_PRESIGN_RND_SIZE (32)
#define MQOM2_PRESIG_SIZE (                                                   \
        MQOM2_PARAM_TAU*BYTE_SIZE_FIELD_BASE(MQOM2_PARAM_ETA*MQOM2_PARAM_MU)  \
      + (MQOM2_PARAM_SALT_SIZE) + (2*(MQOM2_PARAM_DIGEST_SIZE))               \
      + MQOM2_PARAM_PREOPENING_SIZE                                           \
      + MQOM2_PRESIGN_RND_SIZE                                                \
)
...
#endif /* __EXTERNAL_COMMON_OVERRIDE_H__ */
