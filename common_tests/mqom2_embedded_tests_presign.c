...
int main(int argc, char *argv[]){
        ...
        if (ret) {
                printf("Failure: crypto_sign_prepare\r\n");
                panic();
        }
        if(stack_usage >= (uint32_t)MAX_SPRAY_STACK){
                printf("!!!!!! WARNING: stack usage %ld exceeds MAX_SPRAY_STACK = %ld, meaning measurement is lower bound ...\r\n", stack_usage, (uint32_t)MAX_SPRAY_STACK);
        }
        printf("crypto_sign_prepare OK! Took %lld cycles, stack usage = %ld bytes, global tables usage = %ld bytes, sum = %ld bytes\r\r\n", t1-t0, stack_usage, embedded_sram_size, stack_usage + embedded_sram_size);
        print_alloc_usage("crypto_sign_prepare");
        printf("  => crypto_sign_prepare total mem usage: %ld\r\n", stack_usage + embedded_sram_size + alloc_peak_usage);
        ...
        if (ret) {
                printf("Failure: crypto_sign_finalize\r\n");
                panic();
        }
        if(stack_usage >= (uint32_t)MAX_SPRAY_STACK){
                printf("!!!!!! WARNING: stack usage %ld exceeds MAX_SPRAY_STACK = %ld, meaning measurement is lower bound ...\r\n", stack_usage, (uint32_t)MAX_SPRAY_STACK);
        }
        printf("crypto_sign_finalize OK! Took %lld cycles, stack usage = %ld bytes, global tables usage = %ld bytes, sum = %ld bytes\r\r\n", t1-t0, stack_usage, embedded_sram_size, stack_usage + embedded_sram_size);
        print_alloc_usage("crypto_sign_finalize");
        printf("  => crypto_sign_finalize total mem usage: %ld\r\n", stack_usage + embedded_sram_size + alloc_peak_usage);
    ...
    printf("\r\n");
    printf("SIZE of SIGNATURE: %ld, SIZE of PRESIGNATURE: %ld\r\n", CRYPTO_BYTES, CRYPTO_PRESIGNBYTES);
