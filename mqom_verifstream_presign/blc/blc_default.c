...
int BLC_Serialize_Opening_Key_default(const blc_key_default_t* key, uint8_t preopening[MQOM2_PARAM_PREOPENING_SIZE]) {
	memcpy(&preopening[0], key->mseed, MQOM2_PARAM_SEED_SIZE);
	memcpy(&preopening[MQOM2_PARAM_SEED_SIZE], key->partial_delta_x, MQOM2_PARAM_TAU * (BYTE_SIZE_FIELD_BASE(MQOM2_PARAM_MQ_N) - MQOM2_PARAM_SEED_SIZE));
	return 0;
}
...
int BLC_Deserialize_Opening_Key_default(const uint8_t preopening[MQOM2_PARAM_PREOPENING_SIZE], const uint8_t salt[MQOM2_PARAM_SALT_SIZE], const field_base_elt x[FIELD_BASE_PACKING(MQOM2_PARAM_MQ_N)], blc_key_default_t* key) {
	...
	memcpy(key->partial_delta_x, &preopening[MQOM2_PARAM_SEED_SIZE], MQOM2_PARAM_TAU * (BYTE_SIZE_FIELD_BASE(MQOM2_PARAM_MQ_N) - MQOM2_PARAM_SEED_SIZE));

	uint8_t tree_prg_salt[MQOM2_PARAM_SALT_SIZE] = { 0 };
	ret = PRG(tree_prg_salt, 0, key->mseed, MQOM2_PARAM_TAU * MQOM2_PARAM_SEED_SIZE, (uint8_t*)key->rseed, NULL);
	ERR(ret, err);
...
// The implementation of this function is clearly not optimal (since we are doing useless computation),
// but one assumes that a user interested in pre-signature will not use the compilation option BLC_KEEP_ALL_TREES_IN_MEMORY,
// therefore this implementation is just present for completeness.
int BLC_Deserialize_Opening_Key_default(const uint8_t preopening[MQOM2_PARAM_PREOPENING_SIZE], const uint8_t salt[MQOM2_PARAM_SALT_SIZE], const field_base_elt x[FIELD_BASE_PACKING(MQOM2_PARAM_MQ_N)], blc_key_default_t* key) {
	int ret = -1;

	const uint8_t *mseed = &preopening[0];
	...
	ret = BLC_Commit_default(mseed, salt, x, com1, key, x0, u0, u1);
	ERR(ret, err);
