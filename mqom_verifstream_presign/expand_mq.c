...
/* Expand equation in a more "compact" form by exploiting the triangular shape */
int ExpandEquations_tri(const uint8_t mseed_eq[2 * MQOM2_PARAM_SEED_SIZE], field_ext_elt A_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2)], field_ext_elt b_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)]) {
	int ret = -1;
	uint32_t i, j, nb_eq, nf_eq;
	uint8_t prg_salt[MQOM2_PARAM_SALT_SIZE] = { 0 };
	uint8_t *stream = NULL;
	...
	for (i = 3; i < MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU; i += 4) {
		...
		for (z = 0; z < 4; z++) {
			...
			for (j = 0; j < MQOM2_PARAM_MQ_N; j++) {
				/* Fill the jth row of Ai */
				field_ext_parse(&stream[k], (j + 1), &A_hat[i - z][k / sizeof(field_ext_elt)]);
				k += (j + 1) * sizeof(field_ext_elt);
			}
			/* Fill bi */
			field_ext_parse(&stream[k], MQOM2_PARAM_MQ_N, b_hat[i - z]);
...
left_over:
	...
	for (; i < MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU; i++) {
		...
		for (j = 0; j < MQOM2_PARAM_MQ_N; j++) {
			/* Fill the jth row of Ai */
			field_ext_parse(&stream[k], (j + 1), &A_hat[i][k / sizeof(field_ext_elt)]);
			k += (j + 1) * sizeof(field_ext_elt);
		}
		/* Fill bi */
		field_ext_parse(&stream[k], MQOM2_PARAM_MQ_N, b_hat[i]);
