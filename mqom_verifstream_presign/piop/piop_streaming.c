 ...
 * NOTE2: we exploit the triangular shape of the MQ matrices */
typedef field_ext_elt (*MatrixSetMQ)[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2)];
typedef field_ext_elt (*VectorSetMQ)[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)];

static inline void compute_t1(const field_ext_elt A_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2)], const field_base_elt x[FIELD_BASE_PACKING(MQOM2_PARAM_MQ_N)], const field_ext_elt b_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], field_ext_elt t1[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], uint32_t i, piop_cache *cache) {
	if (is_entry_active_piop_cache(cache, i)) {
		get_entry_piop_cache(cache, i, t1);
	} else {
		field_ext_base_mat_mult_tri((field_ext_elt*)A_hat[i], x, t1, MQOM2_PARAM_MQ_N, TRI_INF);
...
static int ComputePz(const field_ext_elt x0_e[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], const field_base_elt x[FIELD_BASE_PACKING(MQOM2_PARAM_MQ_N)], const field_ext_elt A_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2)], const field_ext_elt b_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], field_ext_elt z0[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU)], field_ext_elt z1[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU)], piop_cache *cache) {
	...
	for (i = 0; i < MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU; i++) {
		...
		field_ext_mat_mult_tri((field_ext_elt*)A_hat[i], x0_e, t0, MQOM2_PARAM_MQ_N, TRI_INF);
...
int ComputePAlpha_streaming(const uint8_t com[MQOM2_PARAM_DIGEST_SIZE], const field_ext_elt x0[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], const field_ext_elt u0[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_ETA)], const field_ext_elt u1[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_ETA)], const field_base_elt x[FIELD_BASE_PACKING(MQOM2_PARAM_MQ_N)], const uint8_t mseed_eq[2 * MQOM2_PARAM_SEED_SIZE], field_ext_elt alpha0[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_ETA)], field_ext_elt alpha1[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_ETA)]) {
	...
	_A_hat = (field_ext_elt*)mqom_malloc((MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU) * FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2) * sizeof(field_ext_elt));
	if (_A_hat == NULL) {
		ret = -1;
		goto err;
	}
	...
	ret = ExpandEquations_tri(mseed_eq, A_hat, b_hat);
	ERR(ret, err);
...
err:
	...
	if (_A_hat) {
		mqom_free(_A_hat, (MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU) * FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2) * sizeof(field_ext_elt));
	}
	if (_b_hat) {
...
#define BYTESIZE_MATRICES_A ((MQOM2_PARAM_MQ_M/MQOM2_PARAM_MU) * FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2) * sizeof(field_ext_elt))
#define BYTESIZE_VECTORS_B ((MQOM2_PARAM_MQ_M/MQOM2_PARAM_MU) * FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N) * sizeof(field_ext_elt))
...
piop_data_t *PreparePIOPPublicData(const uint8_t mseed_eq[2 * MQOM2_PARAM_SEED_SIZE], const field_ext_elt y[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU)]) {
	int ret = -1;
	piop_data_t *data = (piop_data_t*) mqom_malloc(sizeof(piop_data_t) + BYTESIZE_MATRICES_A + BYTESIZE_VECTORS_B);
	if (data == NULL) {
		return NULL;
	}
	data->_A_hat = (field_ext_elt*) (((uint8_t*) data) + sizeof(piop_data_t));
	data->_b_hat = (field_ext_elt*) (((uint8_t*) data ) + sizeof(piop_data_t) + BYTESIZE_MATRICES_A);

	MatrixSetMQ A_hat = (MatrixSetMQ) data->_A_hat;
	VectorSetMQ b_hat = (VectorSetMQ) data->_b_hat;
	ret = ExpandEquations_tri(mseed_eq, A_hat, b_hat);
	ERR(ret, err);

	if (y != NULL) {
		memcpy(data->y, y, BYTE_SIZE_FIELD_EXT(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU));
	}
...
err:
	if (ret != 0) {
		if (data) {
			mqom_free(data, sizeof(piop_data_t) + BYTESIZE_MATRICES_A + BYTESIZE_VECTORS_B);
			data = NULL;
		}
	}
	return data;
...
int FinalizePIOPPublicData(piop_data_t* piop_data, const uint8_t com[MQOM2_PARAM_DIGEST_SIZE]) {
	...
	ret = xof_squeeze(&xof_ctx, stream, MQOM2_PARAM_ETA * BYTE_SIZE_FIELD_EXT(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU));
	for (i = 0; i < MQOM2_PARAM_ETA; i++) {
		field_ext_parse(&stream[i * BYTE_SIZE_FIELD_EXT(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU)], MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU, piop_data->Gamma[i]);
	}
#else
	(void)com;
#endif
...
static int ComputePzEval(field_ext_elt r, const field_ext_elt v_x[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], const field_ext_elt A_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N * (MQOM2_PARAM_MQ_N + 1) / 2)], const field_ext_elt b_hat[MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], const field_ext_elt y[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU)], field_ext_elt v_z[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU)]) {
	...
	for (i = 0; i < MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU; i++) {
		...
		field_ext_mat_mult_tri((field_ext_elt*)A_hat[i], v_x, tmp, MQOM2_PARAM_MQ_N, TRI_INF);
...
int RecomputePAlpha_x1(piop_data_t* piop_data, field_ext_elt r, const field_ext_elt alpha1[FIELD_EXT_PACKING(MQOM2_PARAM_ETA)], const field_ext_elt x_eval[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], const field_ext_elt u_eval[FIELD_EXT_PACKING(MQOM2_PARAM_ETA)], field_ext_elt alpha0[FIELD_EXT_PACKING(MQOM2_PARAM_ETA)]) {
	...
	MatrixSetMQ A_hat = (MatrixSetMQ) piop_data->_A_hat;
	VectorSetMQ b_hat = (VectorSetMQ) piop_data->_b_hat;

	ret = ComputePzEval(r, x_eval, A_hat, b_hat, piop_data->y, v_z);

#if MQOM2_PARAM_WITH_STATISTICAL_BATCHING == 1
	uint32_t i;
	for (i = 0; i < MQOM2_PARAM_ETA; i++) {
		field_ext_vect_pack(
		    field_ext_vect_mult(piop_data->Gamma[i], v_z, MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU),
		    v_alpha, i
		);
	}
	field_ext_vect_add(v_alpha, u_eval, v_alpha, MQOM2_PARAM_ETA);
	field_ext_constant_vect_mult(r, alpha1, alpha0, MQOM2_PARAM_ETA);
	field_ext_vect_add(v_alpha, alpha0, alpha0, MQOM2_PARAM_ETA);
#else
	field_ext_vect_add(v_z, u_eval, v_alpha, MQOM2_PARAM_ETA);
	field_ext_constant_vect_mult(r, alpha1, alpha0, MQOM2_PARAM_ETA);
	field_ext_vect_add(v_alpha, alpha0, alpha0, MQOM2_PARAM_ETA);
#endif
...
void CleanPIOPPublicData(piop_data_t *piop_data) {
	if (piop_data) {
		mqom_free(piop_data, sizeof(piop_data_t) + BYTESIZE_MATRICES_A + BYTESIZE_VECTORS_B);
	}
}
...
int RecomputePAlpha_streaming(const uint8_t com[MQOM2_PARAM_DIGEST_SIZE], const field_ext_elt alpha1[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_ETA)], const uint16_t i_star[MQOM2_PARAM_TAU], const field_ext_elt x_eval[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_MQ_N)], const field_ext_elt u_eval[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_ETA)], const uint8_t mseed_eq[2 * MQOM2_PARAM_SEED_SIZE], const field_ext_elt y[FIELD_EXT_PACKING(MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU)], field_ext_elt alpha0[MQOM2_PARAM_TAU][FIELD_EXT_PACKING(MQOM2_PARAM_ETA)]) {
	...
	piop_data_t *piop_data = PreparePIOPPublicData(mseed_eq, y);
	if (piop_data == NULL) {
		return -1;
	}
	ret = FinalizePIOPPublicData(piop_data, com);
	ERR(ret, err);

	for (e = 0; e < MQOM2_PARAM_TAU; e++) {
		field_ext_elt r = get_evaluation_point(i_star[e]);
		ret = RecomputePAlpha_x1(piop_data, r, alpha1[e], x_eval[e], u_eval[e], alpha0[e]);
		ERR(ret, err);
	}

	ret = 0;
err:
	CleanPIOPPublicData(piop_data);
	return ret;
}
