...
#define B_SIGN_PREPARE_ALGO 3
#define B_SIGN_FINALIZE_ALGO 4
...
#define MLEN 32
		...
		uint8_t presig[CRYPTO_PRESIGNBYTES];
		unsigned long long presiglen = 0;
		btimer_start(&timers_algos[B_SIGN_PREPARE_ALGO]);
		ret = crypto_sign_prepare(presig, &presiglen, sk);
		btimer_end(&timers_algos[B_SIGN_PREPARE_ALGO]);
		btimer_count(&timers_algos[B_SIGN_PREPARE_ALGO]);
		timer_pow2[B_SIGN_PREPARE_ALGO] += pow(btimer_diff(&timers_algos[B_SIGN_PREPARE_ALGO]), 2) / nb_tests;
		if (ret) {
			printf("Failure (num %d): crypto_sign_prepare\n", i);
			continue;
		}

		// Finalize the signature
		btimer_start(&timers_algos[B_SIGN_FINALIZE_ALGO]);
		ret = crypto_sign_finalize(sm, &smlen, m, MLEN, presig, presiglen, sk);
		btimer_end(&timers_algos[B_SIGN_FINALIZE_ALGO]);
		btimer_count(&timers_algos[B_SIGN_FINALIZE_ALGO]);
		timer_pow2[B_SIGN_FINALIZE_ALGO] += pow(btimer_diff(&timers_algos[B_SIGN_FINALIZE_ALGO]), 2) / nb_tests;
		if (ret) {
			printf("Failure (num %d): crypto_sign_finalize\n", i);
			continue;
		}

		// Verify/Open the 2-step signature
		...
		if (ret) {
			printf("Failure (num %d): crypto_sign_open (with pre-signature)\n", i);
			continue;
		}
	...
	printf(" - Sign (Prepare):    %.2f ms (std=%.2f)\n",
	       btimer_get(&timers_algos[B_SIGN_PREPARE_ALGO]),
	       std_timer[B_SIGN_PREPARE_ALGO]
	      );
	printf(" - Sign (Finalize):    %.2f ms (std=%.2f)\n",
	       btimer_get(&timers_algos[B_SIGN_FINALIZE_ALGO]),
	       std_timer[B_SIGN_FINALIZE_ALGO]
	      );
	printf("\n");
	...
	printf(" - Sign (Prepare):  %.2f cycles\n", btimer_get_cycles(&timers_algos[B_SIGN_PREPARE_ALGO]));
	printf(" - Sign (Finalize):  %.2f cycles\n", btimer_get_cycles(&timers_algos[B_SIGN_FINALIZE_ALGO]));
	printf("\n");
#endif
